...
from constants import BASE_MODEL, LORA_MODULE_NAMES, ADAPTERS_DIR, VALIDATION_CONFIDENCE
...
validation_dataset = [x for x in mmlu['validation']]

confidence_info = {'samples': validation_dataset}

batches = [validation_dataset[i * batch_size:(i + 1) * batch_size] for i in
           range((len(validation_dataset) + batch_size - 1) // batch_size)]
for adapter_name in ["None"] + LORA_MODULE_NAMES:
    ...
    with open(VALIDATION_CONFIDENCE, 'w', encoding='utf8') as outfile:
